...
bool UnitreeJointController::init(
  ...
  sub_cmd =
      n.subscribe("command", 20, &UnitreeJointController::setCommandCB, this);

  // pub_state = n.advertise<unitree_legged_msgs::MotorState>(name_space +
  // "/state", 20); Start realtime state publisher
  controller_state_publisher_.reset(
      new realtime_tools::RealtimePublisher<unitree_legged_msgs::MotorState>(
...
void UnitreeJointController::update(const ros::Time &time,
  ...
  currentVel = computeVel(currentPos, (double)lastState.q, (double)lastState.dq,
                          period.toSec());
